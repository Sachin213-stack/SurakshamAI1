...
class SentinelService extends ChangeNotifier {
  ...
  Future<void> loadHistory() async {
    try {
      final res = await http.get(Uri.parse('$_apiUrl/alerts/history?page_size=50&device_id=$_deviceId'));
      if (res.statusCode == 200) {
  ...
  Future<void> loadAnalytics() async {
    try {
      final res = await http.get(Uri.parse('$_apiUrl/analytics?device_id=$_deviceId'));
      if (res.statusCode == 200) {
  ...
  // ── Settings ──────────────────────────────────────────────────────────────
  Future<void> saveSettings(String apiUrl, String deviceId) async {
    _apiUrl = apiUrl;
    _deviceId = deviceId;
    final prefs = await SharedPreferences.getInstance();
